...
function getInfo() {
  return inject('js/index.js', 'js', {Wappalyzer: Wappalyzer, Utils: Utils})
}
const Content = {
  cache: {},
  language: '',
  ...
  post2Local: function(data) {
    ...
    fetch("http://127.0.0.1:8000/parseData/", {
      method: "POST",
      mode: "no-cors",
...
if (/complete|interactive|loaded/.test(document.readyState)) {
  ...
  async function sendToParser() {
    const resolved = await Content.driver('getDetections')
    console.log('[!] length: ', Object.keys(resolved).length)
    console.log('[!] Sending the POST Request')
    const url = window.location.href
    const post_url = url.match(/^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:\/\n]+)/im)[1];
    console.log('Posting to URL: ', post_url)
    Content.post2Local({'url':post_url,'technologies':resolved})
  }
  console.log('Setting interval for the sendToParser function')
  setInterval(sendToParser, 1000)
} else {
